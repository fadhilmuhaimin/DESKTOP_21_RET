...
class ItemGridAdapter(private val items: MutableList<JSONObject>) : RecyclerView.Adapter<ItemGridAdapter.ItemGridHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemGridHolder {
        val binding = WidgetCardItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    class ItemGridHolder(val binding: WidgetCardItemBinding) : RecyclerView.ViewHolder(binding.root) {

        fun setContent(json: JSONObject){
            binding.itemNameTv.text = json.getString("name")
            binding.itemDescTv.text = json.getString("description")
            binding.itemPriceTv.text = "Rp. ${json.getInt("price")},00"
